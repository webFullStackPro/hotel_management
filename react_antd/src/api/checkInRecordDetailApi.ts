...
export default class checkInRecordDetailApi {
  static checkInRecordDetails: CheckInRecordDetail[] = [
  ...
  static async save (checkInRecordDetailForm: CheckInRecordDetailForm): Promise<Result<void>> {
  ...
  static async findById (id: number): Promise<Result<CheckInRecordDetail>> {
    console.log('checkInRecordDetailApi findById id', id)
    let target = {}
    for (const a of checkInRecordDetailApi.checkInRecordDetails) {
      if (a.id === id) {
        target = a
      }
    }
    const response: Response<CheckInRecordDetail> = await mockApi.operateSuccessfullyWithData(target)
    return response.data
  }
